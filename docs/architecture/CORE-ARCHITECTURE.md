...
`tools/rasterize_font.py` from the real Press Start 2P and VT323 TTF
files at the fixed pixel sizes established from the design mockups:

| Asset | Source | Size | Role |
|---|---|---|---|
| `tezos_pressstart2p_16` | Press Start 2P | 16px | App headers, logo (placeholder pending MEKmode) |
| `tezos_pressstart2p_32` | Press Start 2P | 32px | Hero display (idle clock) (placeholder pending MEKmode) |
| `tezos_mekmono_16` | MonoMEK (CC0, Michael Alexander/MEK.txt) | 16px | Status bar, softkeys, small labels |
...
**Pending swap**: MonoMEK (body/menu role) is confirmed CC0-licensed by
its creator — ready to replace VT323 as soon as the actual OTF file is in
hand (see `docs/design-system/PALETTE.md`). MEKmode is a candidate for
the display/header role currently filled by Press Start 2P, pending its
own licensing confirmation.

**Re-running the rasterizer** (e.g. to swap in MonoMEK once the file is
available):
```
...
not a technical one — see `docs/design-system/ICONS.md`), MEKmode for
the display/header role (Press Start 2P remains the placeholder pending
a workaround), T9 text-entry widget (needed by Settings, Messenger,
Scanner, Discover — doesn't exist yet), popups/system messages/alerts
